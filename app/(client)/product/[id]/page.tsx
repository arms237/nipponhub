...
export default function ProductDetails() {
  ...
  const [currentImage, setCurrentImage] = useState<string | StaticImageData | undefined>();
  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center py-12">
        <h1 className="text-2xl font-bold text-error mb-4">Produit non trouvé</h1>
        <p className="text-gray-600 mb-6">Désolé, nous n'avons pas trouvé le produit que vous recherchez.</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-6">
              ...
              {product.imgSrc.length > 1 && (
                <div className="flex gap-2 mt-4">
                  {product.imgSrc.map((img, index) => (
                    <div 
                      key={index}
                      className={`w-16 h-16 border-2 cursor-pointer overflow-hidden rounded-md ${currentImage === img ? 'border-primary' : 'border-gray-200'}`}
            ...
            <div className="flex flex-col justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mb-2">Manga: {product.manga}</p>
                ...
                <div className="mt-4">
                  {product.variations.map((variation) => (
                    <div key={variation.name} className="mb-4">
                      <h3 className="text-sm font-medium text-gray-600 mb-2">{variation.name}</h3>
                      <div className="flex flex-wrap gap-2">
                        {variation.variants.map((variant) => (
                          <button
                            key={variant.id}
                            onClick={() => handleVariantChange(variation.name, variant)}
                            className={`px-4 py-2 border rounded-md text-sm font-medium transition-colors ${selectedVariants[variation.name]?.id === variant.id 
              ...
              <div className="mt-8">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <span className="text-3xl font-bold text-primary">{currentPrice} XAF</span>
                    {/* Afficher un badge de stock */}
                    <div className="mt-2">
                      {isInStock !== undefined && isInStock > 0 ? (
                        <span className="inline-block bg-green-100 text-green-800 text-xs px-2 py-1 rounded-full">En stock</span>
                      ) : (
                        <span className="inline-block bg-red-100 text-red-800 text-xs px-2 py-1 rounded-full">Hors stock</span>
                      )}
                    </div>
                  </div>
